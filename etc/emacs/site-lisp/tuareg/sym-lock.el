...
;; removed use-fonts check (due to incomatibilities)
;;   by Albert Cohen <Albert.Cohen@inria.fr> 9-00
;; new after-change/pre-idle policy by Albert Cohen 6-01
;; disable M$ Windows (symbol font problem) and fixed movement bug
;;   (incompatibile atomic-extents and paren) by Albert Cohen 8-01
...
face's extent will become atomic."
  (if (not (and (fboundp 'console-type) (or (eq (console-type) 'x)
					    (eq (console-type) 'gtk))))
